...
def optimize_geometry(input_xyz_file, output_xyz_file, charge, multiplicity):
    subprocess.run(['xtb', input_xyz_file, '--opt', '--gfn2', '--chrg', str(charge), '--uhf', str(multiplicity-1), '--tight'], check=True)
...
def optimize_for_states(input_xyz_file):
    ...
    for state in states:
        ...
        optimize_geometry(input_xyz_file, output_xyz_file, charge, multiplicity)
